...
(() => {
  ...
  function segment(series) {
    ...
    for (const r of series) {
      ...
      const usable = dn !== null && typeof r.p50Ms === 'number';
  ...
  function drawChart() {
    ...
    wireHover(host, pts, X, Y, PAD, ih);
    buildTable(pts);
  }

  /* ----------------------------------------------------------------- hover */

  function wireHover(host, pts, X, Y, PAD, ih) {
    const svg = host.querySelector('svg');
    const guide = host.querySelector('#fc-guide');
    const dot = host.querySelector('#fc-dot');
    ...
    const move = (ev) => {
      ...
      const px = X(dayNum(best.date)), py = Y(best.p50Ms);
      ...
      box.innerHTML =
        `<div class="r-date">${esc(fmtDayYear(best.date))}</div>` +
        row('p90', fmtSec(best.p90Ms)) +
        row('median', fmtSec(best.p50Ms), 'r-p50') +
        row('p10', fmtSec(best.p10Ms)) +
        row('tx', fmtInt(best.samples));
      ...
      box.style.top = Math.max(4, py * scale - box.offsetHeight - 14) + 'px';
    };
  ...
  function init() {
    ...
    if (host) {
      const ro = new ResizeObserver(() => {
        ...
        lastW = w;
        clearTimeout(t);
        t = setTimeout(() => { if (state.data) drawChart(); }, 120);
      });
      ro.observe(host);
    }
